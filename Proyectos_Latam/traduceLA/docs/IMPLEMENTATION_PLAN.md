# Plan de Implementación — TraduceLA
...
- [ ] Modelos a crear: `text`, `voice`, `session`, `history`, `languages`, `dialects`.
...
## Endpoints actuales
- `POST /api/v1/translate/text` — Traducir texto
- `POST /api/v1/translate/voice` — Transcribir audio y traducirlo (mock)
- `POST /api/v1/conversation/session` — Crear sesion de conversacion bilingue
- `GET /api/v1/history` — Historial reciente
- `GET /api/v1/languages` — Lista de idiomas y variantes LATAM soportadas
- `GET /api/v1/dialects` — Dialectos regionales soportados
